...
function enableDragMode(el) {
  ...
  function getOthers() {
    return Array.from(document.querySelectorAll('body *:not(script):not(style):not(link):not(#sb-context-menu)')).filter(other => {
      if (other === el || other.contains(el) || el.contains(other)) return false;
      const r = other.getBoundingClientRect();
      return r.width > 5 && r.height > 5 && r.top < window.innerHeight && r.bottom > 0;
    });
  }

  function showDragGuides(rect) {
    clearDragGuides();
    ...
    if (Math.abs(cx - vpCx) < SNAP) addDragGuide('v', vpCx, '#9C27B0');
    if (Math.abs(cy - vpCy) < SNAP) addDragGuide('h', vpCy, '#9C27B0');

    getOthers().forEach(other => {
      const r = other.getBoundingClientRect();
      const ocx = r.left + r.width / 2;
      const ocy = r.top + r.height / 2;
      if (Math.abs(cy - ocy) < SNAP) addDragGuide('h', ocy, '#2196F3');
      if (Math.abs(rect.top - r.top) < SNAP) addDragGuide('h', r.top, '#ff4081');
      if (Math.abs(rect.bottom - r.bottom) < SNAP) addDragGuide('h', r.bottom, '#ff4081');
      if (Math.abs(cx - ocx) < SNAP) addDragGuide('v', ocx, '#2196F3');
      if (Math.abs(rect.left - r.left) < SNAP) addDragGuide('v', r.left, '#ff4081');
      if (Math.abs(rect.right - r.right) < SNAP) addDragGuide('v', r.right, '#ff4081');
    });
  }

  function addDragGuide(type, pos, color) {
    const line = document.createElement('div');
    line.className = 'sb-drag-guide';
    line.style.cssText = `position:fixed;background:${color};z-index:99996;pointer-events:none;opacity:0.8;`;
    if (type === 'h') { line.style.cssText += `height:1px;left:0;right:0;top:${pos}px;`; }
    else { line.style.cssText += `width:1px;top:0;bottom:0;left:${pos}px;`; }
    document.body.appendChild(line);
  }

  function clearDragGuides() {
    document.querySelectorAll('.sb-drag-guide').forEach(g => g.remove());
  }

  function snapTo(newLeft, newTop) {
    ...
    let sLeft = newLeft, sTop = newTop;

    if (Math.abs(cx - vpCx) < SNAP) sLeft = newLeft + (vpCx - cx);
    if (Math.abs(cy - vpCy) < SNAP) sTop = newTop + (vpCy - cy);

    getOthers().forEach(other => {
      const r = other.getBoundingClientRect();
      const ocx = r.left + r.width / 2, ocy = r.top + r.height / 2;
      if (Math.abs(cy - ocy) < SNAP) sTop = newTop + (ocy - cy);
      if (Math.abs(rect.top - r.top) < SNAP) sTop = newTop + (r.top - rect.top);
      if (Math.abs(rect.bottom - r.bottom) < SNAP) sTop = newTop + (r.bottom - rect.bottom);
      if (Math.abs(cx - ocx) < SNAP) sLeft = newLeft + (ocx - cx);
      if (Math.abs(rect.left - r.left) < SNAP) sLeft = newLeft + (r.left - rect.left);
      if (Math.abs(rect.right - r.right) < SNAP) sLeft = newLeft + (r.right - rect.right);
    });
  ...
  function onMove(e) {
    ...
    const snapped = snapTo(newLeft, newTop);
    el.style.left = snapped.left + 'px';
    el.style.top = snapped.top + 'px';
    showDragGuides(el.getBoundingClientRect());
  }
  function onUp() {
