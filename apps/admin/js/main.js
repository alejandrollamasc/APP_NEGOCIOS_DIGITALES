...
const DEFAULT_PAGES_CHATGPT = [
  { id: 'home', label: 'Bienvenida - Plan', path: '/' },
  { id: 'step1', label: 'Detalle coberturas', path: '/?step=1' },
  { id: 'step2', label: 'Datos del titular', path: '/?step=2' },
  { id: 'step3', label: 'Datos del hogar', path: '/?step=3' },
  { id: 'step4', label: 'Confirme su compra', path: '/?step=4' },
  { id: 'step5', label: 'Frecuencia de pago', path: '/?step=5' },
  { id: 'step6', label: 'Bienvenida', path: '/?step=6' }
];
...
async function renderProjectSelector(app) {
  ...
  const TEMPLATES = [
    ...
    { id: 'chatgpt', name: 'Chatgpt', description: 'Experiencia personalizable libre', icon: '🚀' }
  ];
  app.innerHTML = `
  ...
  document.querySelectorAll('.toolbar__device-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (deviceMode === 'mobile') {
        ...
        iframe.style.width = 'min(375px, 90%)';
        iframe.style.height = 'calc(100% - 40px)';
        iframe.style.maxWidth = 'min(375px, 90%)';
...
async function loadPagesOrder() {
  ...
  const defaults = versionStore.template === 'hogar' ? DEFAULT_PAGES_HOGAR : versionStore.template === 'chatgpt' ? DEFAULT_PAGES_CHATGPT : DEFAULT_PAGES_SALUD;
  PAGES.length = 0;
...
async function applyChanges(user) {
  ...
  if (pending.length === 0) { 
    // Even with no pending changes, regenerate export with latest snapshot
    triggerExportInBackground();
    hideSavingModal(); 
    return; 
  }
  ...
  try {
    ...
    updatePendingCount();
    // Auto-generate export for developers immediately after saving
    await triggerExportInBackground();
    hideSavingModal();
...
// Regenerate the export folder so developers always have the latest code
async function triggerExportInBackground() {
  if (!versionStore.projectId) return;
  try {
    await fetch(`http://localhost:4001/api/projects/${versionStore.projectId}/export`, { method: 'POST' });
  } catch {
    // Export failed silently — non-blocking, will retry on next save
  }
}
...
function scaleIframeToFit() {
  // Ensure mobile mode iframe respects container size
  const iframe = document.getElementById('live-iframe');
  const wrap = document.getElementById('iframe-wrap');
  if (!iframe || !wrap) return;
  if (deviceMode === 'mobile') {
    const containerWidth = wrap.clientWidth;
    const mobileWidth = Math.min(375, containerWidth * 0.9);
